...
Version: 2.1
Project Type: Next.js 16 + React 19 + Payload CMS 3.x (TypeScript)
...
src/
  app/
    ...
    (payload)/    → Admin panel and API routes (DO NOT MODIFY)
  ...
  hooks/          → React hooks and Payload hooks
  lib/            → Utility functions and shared logic
  plugins/        → Payload plugins configuration
  seed/           → Database seed scripts
  utilities/      → Helper functions (getURL, formatDateTime, etc.)
...
- Fetch from Payload using `getPayload()` utility
- Handle errors with try/catch and fallback content

**src/app/(payload)/:**
- Admin panel and API routes (auto-generated)
- DO NOT manually edit these files
- Changes are managed through collections/globals configuration
...
- Each collection has its own file or directory
- Follow existing patterns (see Users, Pages, Posts)
- Always include proper access control

**src/blocks/:**
- Content blocks for Pages layout field
- Each block has: config.ts, Component.tsx, index.ts
- Register new blocks in src/blocks/index.ts

**src/globals/:**
- Payload global configurations (SiteSettings)
- Header and Footer have their own directories with components

**src/access/:**
- Reusable access control functions
- Use `admins`, `authenticated`, `authenticatedOrPublished`, `adminsOrSelf`

**src/utilities/:**
- Helper functions (getURL, formatDateTime, generateMeta, etc.)
- Each utility should have tests

**src/hooks/:**
- Payload hooks (formatSlug, populatePublishedAt, revalidatePath)
- React hooks (use-mobile.ts)
- Payload hooks are server-side, React hooks may be client-side

**src/lib/:**
- Shared utilities and environment validation
- Contains env.ts for environment variable validation
...
## 5. PAYLOAD CMS PATTERNS (IMPORTANT)

This project uses Payload CMS 3.x with PostgreSQL. Follow these patterns:

### Creating Collections

Collections are defined in `src/collections/`. Follow existing patterns:
...
Blocks are reusable content sections for Pages. Each block needs:

1. `config.ts` - Block definition with slug and fields
2. `Component.tsx` - React component for rendering
3. `index.ts` - Exports for config and component

Register new blocks in `src/blocks/index.ts` and add to Pages collection layout field.

### Creating Globals

Globals are single documents (like SiteSettings). Create in `src/globals/` or as directories with components (like Header, Footer).

### Access Control

Use reusable access functions from `src/access/`:
- `authenticated` - Requires authenticated user
- `authenticatedOrPublished` - Public read for published, auth for edit
- `admins` - Admin role only
- `adminsOrSelf` - Admins or the document owner

### Data Fetching

Use the `getPayload()` utility for server-side data fetching:
...
Use the `RichText` component from `src/components/RichText.tsx` for rendering Lexical content:
...
The project builds WITHOUT a database connection:
- Payload handles missing DATABASE_URL gracefully during builds
- All data fetching has try/catch with fallback content
- This allows CI/CD builds without database access

### Regenerating Types

After changing collections, globals, or blocks:
```bash
pnpm generate:types
```

This updates `src/payload-types.ts` with latest schema.

---

## 6. TESTING REQUIREMENTS (MANDATORY, NOT OPTIONAL)
...
- Create a new component → write component tests
- Create a new function → write function tests
- Create a new API route → write API tests
- Create a new hook → write hook tests
- Modify existing functionality → update or add tests
...
## 7. LINTING & TYPE CHECKING (MANDATORY AFTER EVERY CHANGE)
...
## 16. DEPLOYMENT WORKFLOW (CRITICAL)
